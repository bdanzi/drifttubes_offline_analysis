...
struct wave {
	...
	float nnIntegInR(float cut=0.03) {//0.05 proto_cosmic
	...
	void fillWave(std::vector<__uint16_t> &arr, int maxDim=-1) {
		...
		for (int ipt=0; ipt<nPt; ++ipt) {
			Y.push_back(1.0/4096.0*arr[ipt]-0.5);
			...
			if (ipt==99) {
				bsln=integ*0.01;           //diviso 100 per ottenere il valore medio.
				rms=sqrt(rms*0.01-bsln*bsln);
			}
...
//histo che verranno riempiti con wave non filtrate 
	...
	hstPerCh(int Ch=0, int SF=0) {
		...
		if (SF==0) {//senza smooth
			...
			hNPeaks = new TH1F (Form("hNPeaks_ch%d",Ch),Form("N Peaks found - Ch %d",Ch),100,0,100);
			...
			hNPeaks_1 = new TH1F (Form("hNPeaks_1_ch%d",Ch),Form("N Peaks found over noise - Ch %d",Ch),100,0,100);
