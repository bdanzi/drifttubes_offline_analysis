...
void ReadSglChannel_test_NovTestBeam(){
  ...
  if(isInteractive){
   ...
   if (stat(Form("/lustrehome/bdanzi/TestBeam2022/analysistestbeam2022/analysis_2021/drifttubes_offline_analysis/testbeam_analysis/Plots/%s/",fname.Data()), &st) == -1) {
	...
	epc_2cm = epc_2cm/(float) Channel_2cm_size;
	rms_epc_2cm = rms_epc_2cm/(float) Channel_2cm_size;
	
	integral_2cm = (integral_2cm/(float) Channel_2cm_size);
	rms_integral_2cm = (rms_integral_2cm/(float) Channel_2cm_size);
	...
	mean_clusters_2cm = mean_clusters_2cm/(float) Channel_2cm_size;
	rms_clusters_2cm = rms_clusters_2cm/(float) Channel_2cm_size;
