  ...
  for (Long64_t jentry=firstEv; jentry<lastEv;jentry++) {
	...
	if (point.first == channel && !isTrg) {
	  ...
	  if (saveEvents) {
	    ...
	    //nTriggerChannels= nTriggerChannels -1;
	    }
	    tmpCv.back()->cd(channel-nTriggerChannels);
	    //cout<< "nTriggerchannels"<<nTriggerChannels<<" event "<<jentry<<endl;
	    ...
	    tmpWaves.back()->Draw("AL");
	    counting++;
	    if(counting==(nMaxCh-nTriggerChannels)){
	    tmpCv.back()->Write();
	    }
	    theFile->cd("/");
	  }
	...
	float scaleInt=1.0;		
	    ...
	    if(!isTrg&&Waves[channel].max>0.005/*0.01*/){
