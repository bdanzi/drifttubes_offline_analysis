...
Int_t FindPeaks(Int_t npt, Float_t *amplitude, Float_t sig, Int_t nrise,Int_t checkUpTo, Int_t *pkPos, Float_t *pkHgt) {
 // NPeak = FindPeaks(((wave)Waves[channel]).nPtInR(),&((wave)Waves[channel]).Y[skipFstBin],1.2e-3,6,3,pkPos,pkHgt);
//  Int_t nrise=7;//quanti bin per il rise time del segnale//12 initial value
//  Int_t checkUpTo=2;//quanti bin da controllare//6 initial value
